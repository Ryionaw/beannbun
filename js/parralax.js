...
function Parallax(operator, imageUrl, minHeight, minWidth, animation) {
  ...
  if (imageUrl != "") {
    ...
    console.error(
      "Parallax: Vous ne pouvez pas créer un parallax sans image." +
        "Si cette erreur persiste, il se pourrait que l’emplacement de votre image ne sois pas trouvée en raison d’un chemin d’accès erroné." +
        "Ou que l’attribut {@data-src} soit vide ou non inscrit dans votre fichier HTML"
    );
  }
  ...
  if (animation == "true") {
    window.addEventListener("scroll", () => {
      elements[operator].style.backgroundPositionY =
...
Parallax.prototype = {
  ...
  create: function (operator, imageUrl, minHeight, minWidth) {
    ...
    elements[operator].setAttribute(
      "style",
      defaultValues +
        minHeight +
        minWidth +
  ...
  if (elements[i].getAttribute("data-type") == "parallax") {
    ...
    if (elements[i].getAttribute(dataMinHeight) != null) {
      attributes.minHeight =
        "min-height:" + elements[i].getAttribute(dataMinHeight) + ";";
    } else {
    ...
    if (elements[i].getAttribute(dataMinWidth) != null) {
      attributes.minWidth =
        "min-width:" + elements[i].getAttribute(dataMinWidth) + ";";
    } else {
